...
const progressDefault int64 = 999999 // 預設進度值, 先給個很大的進度值, 後面如果有變更再通過ProgressBar.ChangeMax去改
const progressFactor float32 = 1.2   // 進度值乘數, 多了0.2是要包括可能會出現的額外工作量
...
func NewProgressBar(desc string, writer io.Writer) *progressbar.ProgressBar {
	bar := progressbar.NewOptions64(
		progressDefault,
		progressbar.OptionSetDescription(fmt.Sprintf("%-40s", desc)),
