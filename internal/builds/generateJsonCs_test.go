...
func TestGenerateJsonCs(t *testing.T) {
	suite.Run(t, new(SuiteGenerateJsonCs))
}

type SuiteGenerateJsonCs struct {
	suite.Suite
	workDir    string
	codeStruct []byte
	codeReader []byte
...
func (this *SuiteGenerateJsonCs) SetupSuite() {
...
func (this *SuiteGenerateJsonCs) TearDownSuite() {
	_ = os.RemoveAll(internal.PathJsonCs)
	testdata.RestoreWorkDir(this.workDir)
}

func (this *SuiteGenerateJsonCs) target() *RuntimeStruct {
...
func (this *SuiteGenerateJsonCs) TestGenerateJsonCsStruct() {
	target := this.target()
	assert.Nil(this.T(), generateJsonCsStruct(target))
	testdata.CompareFile(this.T(), target.Named.FileJsonCsStruct(), this.codeStruct)
}

func (this *SuiteGenerateJsonCs) TestGenerateJsonCsReader() {
	target := this.target()
	assert.Nil(this.T(), generateJsonCsReader(target))
	testdata.CompareFile(this.T(), target.Named.FileJsonCsReader(), this.codeReader)
}
