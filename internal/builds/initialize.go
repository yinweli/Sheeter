...
func Initialize(content *Content) error {
	...
	if err != nil {
		return fmt.Errorf("%s: initialize failed, excel can't open", content.StructName())
	} // if

	if excel.GetSheetIndex(content.Sheet) == -1 {
		return fmt.Errorf("%s: initialize failed, sheet not found", content.StructName())
	} // if
	...
	if err != nil {
		return fmt.Errorf("%s: initialize failed, field line not found", content.StructName())
	} // if

	layerLine, err := content.GetColumns(content.LineOfLayer)

	if err != nil {
		return fmt.Errorf("%s: initialize failed, layer line not found", content.StructName())
	} // if

	noteLine, err := content.GetColumns(content.LineOfNote)

	if err != nil {
		return fmt.Errorf("%s: initialize failed, note line not found", content.StructName())
	} // if
	...
	for col, itor := range fieldLine {
		...
		if err != nil {
			return fmt.Errorf("%s: initialize failed: %w", content.StructName(), err)
		} // if
	...
	if pkeyCount > 1 {
		return fmt.Errorf("%s: initialize failed, pkey duplicate", content.StructName())
	} // if

	if pkeyCount <= 0 {
		return fmt.Errorf("%s: initialize failed, pkey not found", content.StructName())
	} // if
