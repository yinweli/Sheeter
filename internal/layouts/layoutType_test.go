...
func (this *SuiteLayoutType) TestMerge() {
	target1 := this.target()
	assert.Nil(this.T(), target1.Begin(this.type1))
	assert.Nil(this.T(), target1.Add("name1", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeA, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target1.Add("name2", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target1.Add("name3", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeB, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target1.Add("name4", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target1.Add("name5", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeC, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target1.Add("name6", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target1.End())

	target2 := this.target()
	assert.Nil(this.T(), target2.Begin(this.type2))
	assert.Nil(this.T(), target2.Add("name1", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target2.Add("name2", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeA, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target2.Add("name3", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target2.Add("name4", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeB, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target2.Add("name5", this.fieldNote, this.fieldField, nil, 0))
	assert.Nil(this.T(), target2.Add("name6", this.fieldNote, this.fieldField, []layers.Layer{{Name: this.typeD, Type: layers.LayerStruct}}, 1))
	assert.Nil(this.T(), target2.End())

	target, err := Merge(target1, target2)
...
func (this *SuiteLayoutType) TestBegin() {
	target := this.target()
	assert.Nil(this.T(), target.Begin(this.type1))
	assert.NotNil(this.T(), target.Begin(this.type2))
}
...
func (this *SuiteLayoutType) TestTypeNames() {
	target := this.target()
	assert.True(this.T(), target.pushType(this.type1, true))
	assert.True(this.T(), target.pushType(this.type2, true))
...
func (this *SuiteLayoutType) TestFieldDetails() {
	target := this.target()
	assert.True(this.T(), target.pushType(this.type1, true))
	assert.True(this.T(), target.pushField(this.field1.Name, this.field1.Note, this.field1.Field, this.field1.Alter, this.field1.Array))
	assert.True(this.T(), target.pushField(this.field2.Name, this.field2.Note, this.field2.Field, this.field2.Alter, this.field2.Array))
	results, reader, err := target.FieldDetails(this.type1)
	assert.Nil(this.T(), err)
	assert.True(this.T(), reader)
	assert.Equal(this.T(), []*Field{this.field1, this.field2}, results)

	target = this.target()
	_, _, err = target.FieldDetails(this.type1)
...
func (this *SuiteLayoutType) TestPushType() {
	target := this.target()
	assert.True(this.T(), target.pushType(this.type1, true))
	assert.False(this.T(), target.pushType(this.type1, true))
}
