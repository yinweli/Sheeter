...
type {{$.ReaderName}} struct {
	Data map[{{$.PrimaryGo}}]*{{$.StructName}} // 資料列表
}
...
// FromData 讀取資料
func (this *{{$.ReaderName}}) FromData(data []byte, clear bool, progress *Progress) error {
	tmpl := map[{{$.PrimaryGo}}]*{{$.StructName}}{}
	...
	if clear || this.Data == nil {
		this.Data = map[{{$.PrimaryGo}}]*{{$.StructName}}{}
	} // if
...
// Get 取得資料
func (this *{{$.ReaderName}}) Get(key {{$.PrimaryGo}}) *{{$.StructName}} {
...
// Keys 取得索引列表
func (this *{{$.ReaderName}}) Keys() (result []{{$.PrimaryGo}}) {
...
{{- end}}
			...
			if err := this.{{.StructName}}.FromData(data, i == 0, this.progress); err != nil {
				this.loader.Error(filename.File(), err)
				result.Store(false)
			} // if
		} // for
