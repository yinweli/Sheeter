...
func (this *SuiteJsonPack) TestJsonPack() {
	expected1, _ := utils.JsonMarshal(map[string]interface{}{
		"1": map[string]interface{}{
			"name1": int32(1),
			"name2": int32(1),
			"name3": []int32{10, 11, 12},
			"name4": "a",
			"name5": []string{"a", "b", "c", "d"},
		},
		"2": map[string]interface{}{
			"name1": int32(2),
			"name2": int32(2),
			"name3": []int32{20, 21, 22},
			"name4": "b",
			"name5": []string{"b", "c", "d", "a"},
		},
		"3": map[string]interface{}{
			"name1": int32(3),
			"name2": int32(3),
			"name3": []int32{30, 31, 32},
			"name4": "c",
			"name5": []string{"c", "d", "a", "b"},
		},
	})
	...
	assert.JSONEq(this.T(), string(expected1), string(result))

	sheet, layout = this.prepare(this.excel, this.sheetSuccess)
	result, err = JsonPack("2", this.lintOfData, sheet, layout)
	assert.Nil(this.T(), err)
	assert.JSONEq(this.T(), string(expected2), string(result))

	sheet, layout = this.prepare(this.excel, this.sheetSuccess)
	result, err = JsonPack("3", this.lintOfData, sheet, layout)
	assert.Nil(this.T(), err)
	assert.JSONEq(this.T(), string(expected3), string(result))
	...
	result, err = JsonPack("1", this.lintOfData, sheet, layout)
	assert.Nil(this.T(), err)
	assert.JSONEq(this.T(), "{}", string(result))
}
...
func (this *SuiteJsonPack) prepare(excelName, sheetName string) (sheet *excels.Sheet, layout *Layout) {
	...
	layout, _ = NewLayout(lineTag, lineName, lineNote, lineField)
	return sheet, layout
}
