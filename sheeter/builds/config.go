...
func (this *Config) Initialize(cmd *cobra.Command) error {
	...
	if flags.Changed(flagTags) {
		if value, err := flags.GetString(flagTags); err == nil {
			this.Global.Tags = value
		} // if
	} // if
...
func (this *Config) Sheet() []Sheet {
	...
	for _, itor := range this.Inputs {
		before, after, ok := strings.Cut(itor, sheeter.SeparateSheet)
		...
		if filepath.Ext(before) != sheeter.ExcelExt {
			continue
		} // if
